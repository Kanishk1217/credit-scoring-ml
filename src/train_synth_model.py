"""Train the served hybrid model on synthetic data, calibrate it, pick cost-based thresholds,
run a fairness audit, and export everything TORCH-FREE for serving.

Fixes applied vs the previously-deployed Taiwan model:
  - drops protected attributes (sex/marriage/education) from scoring entirely
  - 12 months of payment history instead of 6, 150k rows instead of 30k
  - isotonic calibration (previous deploy had none -> predicted PD was ~2x actual)
  - cost-based decision thresholds (5:1) instead of arbitrary 0.2/0.5
  - a fairness audit across gender/region (held out of scoring, used only to audit)

Pipeline:
  1. generate synthetic book (12-month sequences, no protected attrs in scoring)
  2. 4-way split: train / val (early stop) / cal (calibrate + threshold) / test (final, honest)
  3. XGBoost on static features (out-of-fold scores for the fusion — no leakage)
  4. LSTM(seq) + fusion MLP  (PyTorch, training only)
  5. isotonic calibration on `cal`; cost-based decision thresholds (5:1) on `cal`
  6. fairness audit on `test` (demographic parity + equalized odds by gender)
  7. export models/hybrid_xgb.joblib, hybrid_fusion.npz, hybrid_config.json
  8. verify the NumPy forward matches PyTorch to ~1e-7

Run:  uv run python src/train_synth_model.py
"""
import os
...
from xgboost import XGBClassifier  # xgboost MUST be imported before torch (OpenMP conflict)
...
COST_FP = 1.0   # cost of declining a good customer  (ratio 5:1, user-chosen)
HIDDEN = 32

# ---------------- data ----------------
print("Generating synthetic book (150k rows, 12-month sequences, no protected attrs)...")
...
# ---------------- LSTM + fusion (PyTorch, training only) ----------------
...
ytr = tens(y[i_tr]).unsqueeze(1)

model = Hybrid(HIDDEN)
crit = nn.BCEWithLogitsLoss(pos_weight=tens([spw]).float())
opt = torch.optim.Adam(model.parameters(), lr=5e-3)
loader = DataLoader(TensorDataset(seq_t["tr"], sc_t["tr"], ytr), batch_size=512, shuffle=True)
...
print(f"mean predicted PD (test, uncalibrated): {raw_pd('test').mean():.4f}  "
      f"vs actual rate {y[i_test].mean():.4f}")

# ---------------- isotonic calibration (fit on cal, evaluate on test) ----------------
...
print(f"mean predicted PD (test, CALIBRATED): {cal_test.mean():.4f}  vs actual rate {y[i_test].mean():.4f}")

# reliability table (honest check that calibration worked)
...
# ---------------- cost-based thresholds (fit on cal, not test) ----------------
pc = calibrate(raw_pd("cal")); yc = y[i_cal]
ts = np.linspace(0.02, 0.98, 481)
costs = [COST_FN * ((pc < t) & (yc == 1)).sum() + COST_FP * ((pc >= t) & (yc == 0)).sum() for t in ts]
t_decline = float(ts[int(np.argmin(costs))])           # cost-optimal approve/decline boundary
t_approve = round(max(0.05, t_decline * 0.5), 3)       # confidently-safe zone below
thresholds = {"approve": t_approve, "decline": round(t_decline, 3)}
print(f"\ncost-optimal thresholds (5:1 cost ratio): approve < {t_approve}  "
      f"|  review  |  decline >= {round(t_decline, 3)}")
...
print("decision mix on test (NEW model):", mix)

# ---------------- fairness audit (gender/region held OUT of scoring; audit only) ----------------
print("\n=== FAIRNESS AUDIT (gender/region never used for scoring) ===")
gender_test = df["gender"].values[i_test]
region_test = df["region"].values[i_test]
approved = (rec_test != "decline")   # "positive outcome" = not declined
fairness = {}
for attr_name, attr_vals in [("gender", gender_test), ("region", region_test)]:
    ...
    for g in groups:
        ...
        # equalized odds: TPR (approved | actually good) and FPR (approved | actually bad) per group
        good_m = m & (y[i_test] == 0); bad_m = m & (y[i_test] == 1)
        # tpr = "approve the good ones" rate; fpr = "approve the bad ones" rate (want low & equal)
    ...
    dp_gap = round(max(rates) - min(rates), 4)          # demographic parity gap
    ...
    for g, r in rows.items():
        print(f"    group {g}: n={r['n']:>6}  approval_rate={r['approval_rate']:.3f}  "
              f"TPR(good approved)={r['tpr_good_approved']}  FPR(bad approved)={r['fpr_bad_approved']}")

# ---------------- feature ranges for the UI ----------------
ranges = {}
...
joblib.dump(xgb, models / "hybrid_xgb.joblib")
sd = model.state_dict()
np.savez(models / "hybrid_fusion.npz",
...
config = {
    "static_cols": STATIC_COLS, "seq_len": len(SEQ_COLS),
...
# ---------------- verify NumPy == PyTorch ----------------
d = np.load(models / "hybrid_fusion.npz")
...
print("saved: models/hybrid_xgb.joblib, models/hybrid_fusion.npz, models/hybrid_config.json")
